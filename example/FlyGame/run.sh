...
function build(){
    ...
    echo "Compiling..."
    javac -encoding ISO-8859-1 -d obj -classpath src -bootclasspath $PLATFORM  src/com/pg/*.java

    echo "Translating in Dalvik bytecode..."
    $DX --dex --output=classes.dex obj

    echo "Making APK..."
    $AAPT package -f -m -F bin/flygame.unaligned.apk -M AndroidManifest.xml -S res -I $PLATFORM
    $AAPT add bin/flygame.unaligned.apk classes.dex

    echo "Aligning and signing APK..."
    $ZIPALIGN -f 4 bin/flygame.unaligned.apk bin/flygame.apk
    $APKSIGNER sign --ks mykey.keystore bin/flygame.apk
}
...
function clean(){
    ...
    rm -rf bin/*
    rm -rf obj/*
    rm -rf src/com/pg/R.java
}

function program(){
	echo "Launching..."
	adb install -r bin/flygame.apk
	adb shell am start -n com.pg/.MainActivity
}
